...
c.deserialize=function(a){if("undefined"===typeof JSON)throw Error("Your browser does not currently support JSON deserialization.");if(!a)return null;a=JSON.parse(a);return c.fromObject(a)};c.prototype.clone=function(){var a=new c(this.key);a.ctrl=this.ctrl;a.alt=this.alt;a.shift=this.shift;a.meta=this.meta;return a};c.fromObject=function(a){if(!a||!a.key||!a.key.name||!a.key.code)throw Error("Combo.fromObject: Invalid Combo object provided.");var d=new b(a.key.name,a.key.code);a=[b.CTRL,b.ALT,b.SHIFT,
b.META].zipmap([a.ctrl,a.alt,a.shift,a.meta]).filter(function(a){return!0===a[1]}).map(function(a){return a[0]});if(a.length)return new c(d,a);throw Error("Combo.fromObject: Invalid Combo, at least one meta key should be set.");};c.fromEvent=function(a){var d=b.fromCode(a.which),g=new c(d);g.shift=a.shiftKey||d.eq(b.SHIFT);g.alt=a.altKey||d.eq(b.ALT);g.meta=a.metaKey||d.eq(b.META)||d.eq(b.META_RIGHT);g.ctrl=a.ctrlKey||d.eq(b.CTRL);return g};c.fromString=function(a){a=a.split("+");if(1<=a.length){var d=
new c(b.fromName(a[a.length-1]));d.ctrl=-1<a.indexOf("CTRL");d.alt=-1<a.indexOf("ALT");d.shift=-1<a.indexOf("SHIFT");d.meta=-1<a.indexOf("META")||-1<a.indexOf("META_RIGHT");return d}throw Error("Combo.fromString: Invalid string");};c.prototype.eq=function(a){return!a||!(a instanceof c)?!1:this.key.eq(a.key)?this.shift!==a.shift?!1:this.alt!==a.alt?!1:this.ctrl!==a.ctrl?!1:this.meta!==a.meta?!1:!0:!1};c.prototype.isMatch=function(a,d){if(!a)throw Error("Combo.isMatch called without a combo to match against.");
if(this.key.isMeta()){if((this.shift||this.key.eq(b.SHIFT))&&!a.shift||(this.alt||this.key.eq(b.ALT))&&!a.alt||(this.ctrl||this.key.eq(b.CTRL))&&!a.ctrl||(this.meta||this.key.eq(b.META)||this.key.eq(b.META_RIGHT))&&!a.meta)return!1}else if(!d&&!this.key.eq(a.key)||this.shift&&!a.shift||this.alt&&!a.alt||this.ctrl&&!a.ctrl||this.meta&&!a.meta)return!1;return!0};c.prototype.requires=function(a){if("number"!==typeof a)throw Error("Combo.requires: `which` must be a keycode (number)");return this.key.code===
a?!0:this.ctrl&&this.key.eq(b.CTRL)?!0:this.shift&&this.key.eq(b.SHIFT)?!0:this.alt&&this.key.eq(b.ALT)?!0:this.meta&&(this.key.eq(b.META)||key.eq(b.META_RIGHT))?!0:!1};c.prototype.containsMetaKeys=function(){return this.ctrl||this.shift||this.alt||this.meta||this.key.isMeta()};f.Combo=c;h.prototype.get=function(a){return k(this.bindings,function(b){return b.name===a})};h.prototype.add=function(a,b){if(!a||!b)throw Error("Keybindings.add: Invalid arguments provided");if(!(b instanceof c))throw Error("Keybindings.add: `combo` must be an instance of Combo");
...
handler:c});l("Bindings.registerHandler: Handler `"+a+"` registered for `"+b+"` events.")};h.prototype.registerToggle=function(a,b,c){if(3!==arguments.length)throw Error("Keybindings.registerToggle: You must provide all three arguments to this function.");this.handlers.push({name:a,eventType:"keydown",handler:function(){var a=!1;return function(){var f=Array.prototype.slice.call(arguments);a?(a=!1,c.apply(null,f)):(a=!0,b.apply(null,f))}}()});l("Bindings.registerToggle: Toggle `"+a+"` registered.")};
h.prototype.serialize=function(){if("undefined"===typeof JSON)throw Error("Your browser does not support JSON serialization.");return JSON.stringify(this)};h.prototype.deserialize=function(a){if("undefined"===typeof JSON)throw Error("Your browser does not support JSON serialization.");a=JSON.parse(a);if(!a||!a.bindings||a instanceof Array)throw Error("Keybindings.deserialize: Unable to deserialize keybindings");this.bindings=a.bindings.map(function(a){a.combo=c.fromObject(a.combo);return a})};h.prototype.getHandlersForCombo=
function(a,b){var c=this.bindings.filter(function(c){return b?c.combo.isMatch(a,!0):c.combo.isMatch(a)});return this.handlers.filter(function(a){return k(c,function(b){return b.name===a.name})})};f.Bindings=h;return f});
